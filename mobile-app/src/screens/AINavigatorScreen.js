...
import {
  ...
  ChevronLeft, MapPin, Clock, Volume2, ShieldAlert
...
function buildVoiceMsg(zone, lang) {
  ...
  if (lang === 'mr') {
    ...
    const crime = zone.crimeType || 'Danger';
    return `सावधान! ${name} जवळ ${crime} आहे. काळजी घ्या.`;
  }

  if (lang === 'hi') {
    const name = zone.name || 'Danger Zone';
    const crime = zone.crimeType || 'Danger';
    return `सावधान! ${name} के पास ${crime} है। सतर्क रहें।`;
  }
...
export default function AINavigatorScreen({ route, navigation }) {
  ...
  const [isRescue, setIsRescue] = useState(route.params?.rescueMission || false);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (route.params?.rescueMission && route.params?.targetCoords) {
        handleRescueSetup(route.params.targetCoords);
      }
    };
    init();
  ...
  const handleRescueSetup = async (coords) => {
    setIsRescue(true);
    const dest = {
      latitude: coords.latitude,
      longitude: coords.longitude,
      name: `RESCUE: ${coords.name}`
    };
    await selectDestination(dest);
    // Auto-start for rescue
    setTimeout(() => startNavigation(), 1500);
  };
  ...
  const initLocation = async () => {
    ...
    // 🧪 EMULATOR FIX: If we are in America (default emulator location), jump to Sindhudurg
    if (coords.latitude > 30 && coords.latitude < 40 && coords.longitude < -120) {
       coords = { latitude: 16.0361, longitude: 73.5042 };
       console.log('🧪 Emulator detected: Auto-teleporting to Sindhudurg');
    }
  ...
  const selectDestination = async (place) => {
    // Handle both Photon search result or direct coordinate object
    const dest = place.latitude ?
      { ...place } :
      { latitude: parseFloat(place.lat), longitude: parseFloat(place.lon), name: place.display_name.split(',')[0] };
        ...
        <LinearGradient colors={['rgba(0,0,0,0.85)', 'transparent']} style={styles.topGradient}>
          {isRescue && (
            <View style={styles.rescueHeader}>
              ...
              <Text style={styles.rescueHeaderText}>LIVE RESCUE MISSION ACTIVE</Text>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  rescueHeader: { backgroundColor: '#FF1744', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, paddingVertical: 8, borderRadius: 10, marginBottom: 10 },
  rescueHeaderText: { color: '#FFF', fontWeight: '900', fontSize: 12, letterSpacing: 1 },
